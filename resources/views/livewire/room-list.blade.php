...
<div class="roomList">
    ...
    <div class="roomList__header">
        <div>
            <h2>Study Room</h2>
            <p>{{ $roomsCount }} Rooms available</p>
        </div>
        <a class="btn btn--main" href="{{ route('room.create') }}" wire:navigate>
            <svg version="1.1" xmlns="http://www.w3.org/2000/svg" width="32" height="32" viewBox="0 0 32 32">
                <title>add</title>
                <path
        ...
        @foreach ($rooms as $room)
            <div wire:key="room-{{ $room->id }}" class="roomListRoom">
                ...
                <div class="roomListRoom__content">
                    <a href="{{ route('room', $room->slug) }}">{{ $room->name }}</a>
                    <p>
                        {{ $room->description }}
                    </p>
                </div>
                <div class="roomListRoom__meta">
                    <a href="{{ route('room', $room->slug) }}" class="roomListRoom__joined">
    ...
    @else
        <div class="no-rooms-message" style="text-align: center; padding: 2rem; color: #6c757d;">
            <p>No rooms found. Be the first to create one!</p>
            <a href="{{ route('room.create') }}" class="btn btn--main" style="margin-top: 1rem;" wire:navigate>
